...
from .models import Konten, TugasKonten, AkunMedsos, RiwayatMisi
...
def dashboard_uploader(request):
    ...
    # DEBUG: Ambil semua tugas terbaru (Bypass Filter Tanggal)
    tugas_aktif_list = TugasKonten.objects.all().select_related('konten').order_by('-id')[:10]
...
def daftar_konten(request):
    ...
    return render(request, 'konten/konten.html', {
        ...
        'active_platform': platform or 'ALL' # Untuk highlight tombol
    })
